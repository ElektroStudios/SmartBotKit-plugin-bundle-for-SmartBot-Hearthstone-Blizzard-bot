...
#Region " Private Methods "
            ...
            If Not (Me.emotedFirstTurn) AndAlso
                ...
                Dim t As Task = Task.Factory.StartNew(
                    Sub()
                        ...
                        If (queued) Then
                            Bot.Log(
                                $"[Emote Factory] -> Sending emote '{emote}' due to condition: '{ _
        ...
        ''' </summary>
        ''' ----------------------------------------------------------------------------------------------------
        Private Async Sub MaybeDoReplyEmote(emoteReceived As EmoteType)
            ...
            If (Interlocked.Increment(Me.replyCount) <= Me.DataContainer.MaxReplies) Then
                ...
                Dim t As Task = Task.Factory.StartNew(
                    Sub()
                        ...
                        If (queued) Then
                            Bot.Log(
                                $"[Emote Factory] -> Sending emote '{emoteToSend}' as a reply to enemy emote: '{emoteReceived}'.")
                        End If
                    End Sub, TaskCreationOptions.LongRunning)
        ...
        ''' ----------------------------------------------------------------------------------------------------
        <DebuggerStepperBoundary>
        Private Function MaybeSendEmote(percent As Integer, emote As EmoteType) As Boolean
