...
#Region " Public Methods "
        ...
        Public Overrides Sub OnGameEnd()
            ...
            Dim notifyLegend As Boolean =
                (((currentMode = Bot.Mode.Standard) AndAlso (Bot.GetPlayerDatas().GetRank(Bot.Mode.Standard) = 0)) OrElse
                ((currentMode = Bot.Mode.Wild) AndAlso (Bot.GetPlayerDatas().GetRank(Bot.Mode.Wild) = 0)) OrElse
                ((currentMode = Bot.Mode.Twist) AndAlso (Bot.GetPlayerDatas().GetRank(Bot.Mode.Twist) = 0)))
...
#End If
            ...
            If (notifyLegend AndAlso Not Me.isLegendNotified) Then
                ...
                If (Me.DataContainer.PlaySoundFile) Then
                    ...
                    If Not file.Exists() Then
                        Bot.Log($"[Legend Rank Handler] -> Sound file not found: '{file.FullName}'")
                    Else
        ...
        ''' ----------------------------------------------------------------------------------------------------
        ''' <summary>
        ''' Releases all the resources used by this <see cref="LegendRankHandlerPlugin"/> instance.
        ''' </summary>
        ''' ----------------------------------------------------------------------------------------------------
