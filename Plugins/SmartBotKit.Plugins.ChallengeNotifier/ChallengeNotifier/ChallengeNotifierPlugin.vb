...
Namespace ChallengeNotifier
    ...
    ''' ----------------------------------------------------------------------------------------------------
    ''' <summary>
    ''' This plugin will notify you when a friend challenges you for the 'Play a Friend' quest.
    ''' </summary>
    ''' ----------------------------------------------------------------------------------------------------
    ''' <seealso cref="Plugin"/>
    ''' ----------------------------------------------------------------------------------------------------
...
#Region " Public Methods "
        ...
        Public Overrides Sub OnPluginCreated()
            ...
            If (Me.lastEnabled) Then
                Bot.Log("[ChallengeNotifier] Plugin initialized.")
            End If
            Me.stopWatch.Start()
        ...
        Public Overrides Sub OnDataContainerUpdated()
            ...
            If (enabled <> Me.lastEnabled) Then
                If (enabled) Then
                    ...
                    Bot.Log("[ChallengeNotifier] Plugin enabled.")
                Else
                    Me.stopWatch.Reset()
                    Bot.Log("[ChallengeNotifier] Plugin disabled.")
                End If
...
#If DEBUG Then
                    If (Me.DataContainer.SaveScreenshots) Then
                        ...
                        Bot.Log("[ChallengeNotifier] Screenshot Saved. ")
...
#If DEBUG Then
                    For Each match As TemplateMatch In matches
                        Bot.Log("[ChallengeNotifier] *DEBUG* AForge Rectangle: " & match.Rectangle.ToString())
                    Next match
#End If
                    Return matches.Any()
...
#End If
        ...
        Private Sub NotifyNewChallenge(ByVal isGoldChallenge As Boolean)
            ...
            If (Me.DataContainer.PlaySoundFile) Then
                Try
                    Dim filename As String = If(isGoldChallenge, "ChallengeNotifier_Gold.wav", "ChallengeNotifier_Normal.wav")
                    Using player As New SoundPlayer(Path.Combine(SmartBotUtil.PluginsDir.FullName, filename))
                        player.Play()
                    End Using

                Catch ex As Exception
                    Bot.Log("[ChallengeNotifier] Failed to play the sound file. Error message: " & ex.Message)
