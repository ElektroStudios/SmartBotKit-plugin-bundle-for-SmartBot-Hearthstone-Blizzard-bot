...
Namespace PanicButton
    ...
    ''' ----------------------------------------------------------------------------------------------------
    ''' <summary>
    ''' This plugin will terminate SmartBot process when a specified hotkey combination is pressed.
    ''' </summary>
    ''' ----------------------------------------------------------------------------------------------------
    ''' <seealso cref="Plugin"/>
    ''' ----------------------------------------------------------------------------------------------------
    <SuppressMessage("Microsoft.Design", "CA1001:TypesThatOwnDisposableFieldsShouldBeDisposable",
                     Justification:="No really need to implement it since it disposes via the inherited Plugin.Dispose() method. 
                                     The real problem is with the design of the Plugin class, which does not implement IDIsposable iterface but a Dispose() method.")>
        ...
        Public Overrides Sub OnDataContainerUpdated()
            ...
            Else
                If Not (Me.warningIsSent) Then
                    ...
                    Bot.Log("[PanicButton] Any modified settings (including the plugin reactivation) will be applied the next time you start SmartBot.")
                End If
            End If
